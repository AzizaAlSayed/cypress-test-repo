Feature: Make a Favorite Post
  ...
  Scenario: Verify that user can make an article as a favoraite on the article page
      Given The system has an article created by that user 
      And   A user was on the article page
      And   The user logged in with an existing account
      When  The user clicks on favorite button
      Then  The article counter favorite should be equal to (1) in the Articles tab
      And   The article counter favorite should be equal to (1)  in the Favorited tab
